...
ContinueMinimumBattles:
	...
	call GetTrainerAndParty
	call DecypherTrainerAndParty
	predef HealParty
...
.notGymLoss
	...
	; Set a flag if the player lost to an Elite Four member
	ld a, [wCurOpponent]
	cp OPP_ELITE_FOUR
	jp nz, .notEliteFourLoss
	ld a, [wMinBattlesTemp]
	set 5, a
	ld [wMinBattlesTemp], a
.notEliteFourLoss
...
GetMinBattlesGameBlueprintFromGameType:
	...
	; 11x the game type to serve as an array index
	ld b, a
rept 10
	add a, b
endr
	...
	; Load the Elite Four list
	ld a, [hli]
	ld [wMinBattlesEliteFourList], a
	ld a, [hli]
	ld [wMinBattlesEliteFourList + 1], a
	; Load the rival list
	ld a, [hli]
...
.NotGymLeader
	...
	; if Elite Four, load that data here
	cp OPP_ELITE_FOUR
	jr nz, .NotEliteFour
	call GetEliteFourParty
	jp .end
.NotEliteFour
...
GetEliteFourParty:
	; Set the Elite Four Number so music is correct
	ld a, [wTrainerNo]
	inc a
	ld [wGymLeaderNo], a

	ld a, [wTrainerNo]
	; Double the Elite Four index to serve as an array index
	add a, a
	; Load the array index into the bc register
	ld c, a
	ld b, 0
	; index into MinBattlesRedEliteFourData with
	; wTrainerNo * length of trainer_info
	ld a, [wMinBattlesEliteFourList]
	ld l, a
	ld a, [wMinBattlesEliteFourList + 1]
	ld h, a
	add hl, bc
	; Load the trainer class
	ld a, [hli]
